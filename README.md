...
- **对话式检索问答**:支持"什么时候可以拆模?"等自然语言提问;复合问题自动拆成多个子句分别检索,追问可结合上下文。
...
历史记录由 [conversation.py](app/conversation.py) 独立管理,核心是**纯内存、按客户隔离的多轮对话记忆**:服务端用 `Dict[client_key, List[消息]]` 维护每个客户身份的对话上下文,无任何落盘。
...
class Conversation:
    ...
    def _trim(self):       # FIFO:超过 20 条只留最近 20 条
...
`ConversationManager` 为模块级单例,随 Flask 进程存活;每个 `client_key` 一份独立历史,互相不可见。

### 一次请求的时序

1. **请求进来,先取旧历史**:`conv.history_for_llm()` 返回消息列表的拷贝,塞进 `QAContext.history`。此时本轮问答**尚未写入**——LLM 看到的只是"之前聊过的内容"。
2. **引擎内历史被消费两次**(见 `qa_engine.py`):
   - **检索前·追问改写**:倒序找最近一条 user 消息,与当前问题拼接做一次扩展检索(如上一问"5 层哪天浇筑的?" + 当前问"那 B 区呢?");
   - **生成时·多轮对话**:取最后 8 条历史追加到 messages(system prompt 之后、本轮问题之前),LLM 因此能理解指代与省略。
3. **回答完毕才写入本轮问答**:流式模式下边流边攒 `full_answer`,流结束后 `conv.add_user(message)` + `conv.add_assistant(full_answer)` 一次性追加。只记录问答文本,内部检索细节(命中 chunk、分数)不进历史。

### 清除与隔离

- `POST /api/clear` → 清空当前 `client_key` 的历史;前端"清空对话"按钮与**切换客户身份时**均会调用,防止上下文串身份。
- 不同客户各自独立字典条目,历史同样遵循客户隔离。

### 当前限制与扩展方向

- **不持久化**:服务重启即全部丢失,属演示级取舍;扩展可加 `chat_id` 维度(一个客户多个会话)+ SQLite/Redis 持久化 + TTL。
- **无摘要压缩**:长对话靠"最近 20 条 + LLM 只看最近 8 条"控制长度;更优做法是将被截断的部分让 LLM 先总结成一段记忆再拼接。
